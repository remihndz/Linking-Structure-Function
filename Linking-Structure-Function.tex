...
\begin{abstract}
  \textbf{Purpose:} We developed a framework to generate virtual vasculature of the retina, from the central retinal vessels down to the macular capillaries, alongside a haemodynamics model which was used to test the associations between vascular morphology as delineated by optical coherence tomography (OCTA) and retinal perfusion.
  ...
  Associations between total retinal blood flow and macular flow ratio and vascular morphology are derived as Spearman rank coefficients and uncertainty from input parameters quantified.
  
  \textbf{Results:} A virtual cohort of \SI{200}{} healthy vasculatures were generated.
  Several morphological metrics were in good agreement with pusblished values.
  ...
  The macular flow ratio was not correlated with any morphological metrics.
...
From the haemodynamics simulations, two variables were extracted to quantify macular perfusion: `retinal blood flow', defined as the volumetric flow rate of blood entering the retina, and the `macular flow ratio' defined as the percentage of retinal blood flow entering the macula.
...
From the haemodynamics simulations, the mean $\pm$ standard deviation for retinal blood flow, blood velocity in the CRA and macular flow ratio were $\SI[per-mode=symbol]{14.7 \pm 5.6}{\micro\litre\per\minute}$ (range: \SIrange[per-mode=symbol]{3.3}{40.4}{\micro\litre\per\minute}), $\SI[per-mode=symbol]{1.2 \pm 0.2}{\cm\per\second}$ (range: \SIrange[per-mode=symbol]{0.5}{2}{\cm\per\second}) and $\SI{18.4 \pm 3.5}{\percent}$ (ranges: \SIrange{12.0}{27.8}{\percent}).
...
The correlations with the macular flow ratio were all below \SI{0.25}, indicating weak or no correlations.
...
\subsubsection{macular flow ratio is independent of OPP}\label{sec:UQ}
...
Across all scenarios, the mean retinal blood flow varied between \SI{-30.3}{\percent} and \SI{25.2}{\percent} of baseline values whereas macular flow ratio varied between \SI{-17.8}{\percent} and \SI{73.1}{\percent}.
The results for each scenario tested are shown in Table~\ref{tab:sup-UQranges}.
Variation in OPP had almost no effects on macular flow ratio (Pearson's $R^2<10^{-3}$ for all values of $\mathcal R$) but was linearly correlated with retinal blood flow ($R^2>0.81$ for all values of $\mathcal R$).
...
In Section~\ref{sec:UQ}, we quantified the effects of the two parameters of the haemodynamics model on total retinal blood flow and macular flow ratio.
Increasing $\mathcal R$ is similar to gradually closing the connections to/from the vascular compartment.
Therefore, flow is shunted towards the macula and macular flow ratio increases.
Decreasing $\mathcal R$ has an opposite effect, however, the macular flow ratio will eventually reach a plateau when the CRA reaches its maximum capacity in term of blood flow: regardless of the resistance of paths outside the macula, blood will flow through the macula and total retinal blood flow is bounded by physical constraints.
...
\toprule
 &
   &
     \multicolumn{4}{l}{\begin{tabular}[c]{@{}l@{}}Total retinal blood flow\\ ($\SI{}{\micro\litre\per\minute}$)\end{tabular}} & \multicolumn{4}{l}{macular flow ratio (\%)} \\
...
\resizebox{\textwidth}{!}{%
  \begin{tabular}{lllllll}
    ...
    macular flow ratio        & 0.89     & 0.89      & 0.88      & 0.00       & 0.00        & 0.00       \\
    \bottomrule
\end{tabular}%
}
...
\begin{figure}[ht!]
  ...
  \caption{Association between macular flow ratio and population parameters. $\rho$ is the Spearman correlation coefficient.}
  \label{fig:sup-MFR}
\end{figure}
