...
For baseline simulations, $\mathcal R=\SI[per-mode=symbol]{5e5}{\mmHg\second\per\mL}$ was selected such that the haemodynamic parameters' distribution across the vasculature is similar to experimental data~\cite{DoblhoffDier2014,Riva1985}.
...
\begin{table}
  ...
  \resizebox*{!}{\dimexpr\textheight-2\baselineskip\relax}{%
    ...
    \\
    % \midrule
    % {$\mathcal R$}                     & {Resistance of the links with the peripheral vascular compartment (\SI[per-mode=symbol]{}{\mmHg\second\per\mL}).}   & {$5\times10^5$}
    % \\
    \bottomrule
...
Other parameters of interest include $N_{seeds}$ for the SVP, $N_{terms}$ for the second stage of the CCO and, to a lesser extent, $l_{fr}$.
